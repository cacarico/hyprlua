...
// Initializes file paths and the watching flag
...
// The core file watching logic using inotify
void FileWatcher::watch() {
    ...
    int watchDesc = inotify_add_watch(inotifyFd, m_directory.c_str(),
                                      IN_MODIFY | IN_CLOSE_WRITE | IN_MOVED_TO | IN_CREATE);
    if (watchDesc < 0) {
        sendNotification(
            "[Hyprlua] Failed to add watch for " + m_directory + ": " + std::string(std::strerror(errno)),
            CHyprColor{1.0, 0.2, 0.2, 1.0},
            5000
        );
        close(inotifyFd);
    ...
    sendNotification(
        "[Hyprlua] Monitoring '" + m_filepath + "' for changes...",
